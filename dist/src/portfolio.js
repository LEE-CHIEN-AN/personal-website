...
function createSummaryTags(project) {
    const techStack = project.techStack ?? [];
    const metaTags = project.tags ?? [];
    if (techStack.length === 0 && metaTags.length === 0)
        return null;
...
function pickPrimaryLinks(links) {
    if (links.length === 0)
        return [];
    const byType = (t) => links.find((l) => l.type === t);
    const picked = [];
    const demo = byType("demo");
    const github = byType("github");
    if (demo)
        picked.push(demo);
    if (github && picked.length < 2)
        picked.push(github);
...
function createProjectLinkChip(link) {
...
function getCoverUrl(project) {
    ...
    const youtubeLink = project.links?.find((l) => l.type === "youtube")?.url ??
