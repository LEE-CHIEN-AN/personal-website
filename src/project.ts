type ProjectLinkType = "github" | "demo" | "design" | "pdf" | "image" | "youtube" | "other" | "apk";
...
function showError(container: HTMLElement, errorState: HTMLElement | null) {
  container.innerHTML = "";
...
function renderProject(container: HTMLElement, project: Project) {
  container.innerHTML = "";
  ...
  if (details) sections.appendChild(details);
...
function renderProjectDetails(project: Project): HTMLElement | null {
  const hasMeta = Boolean(project.date || project.course);
  const topics = project.topics ?? [];
  const hasTopics = topics.length > 0;
  if (!hasMeta && !hasTopics) return null;
...
function renderOverview(project: Project): HTMLElement {
  const section = document.createElement("section");
  section.className = "project-detail-section";
...
function renderContributions(items: string[]): HTMLElement {
  const section = document.createElement("section");
  section.className = "project-detail-section";
...
function renderTags(techStack: string[], tags: string[]): HTMLElement {
  const section = document.createElement("section");
  section.className = "project-detail-section";
...
function renderLinks(links: ProjectLink[]): HTMLElement {
  const section = document.createElement("section");
  section.className = "project-detail-section";
...
function renderEmbeds(embeds: ProjectEmbed[]): HTMLElement {
  const section = document.createElement("section");
  section.className = "project-detail-section";
...
function getRatioClass(ratio: ProjectEmbed["ratio"]): string {
  switch (ratio) {
    case "4-3":
